...
- #360：工作区 `work/` 领地布局。initialize/create 脚手架 `work/README.md`（已有工作区不加强制迁移）。hire 创建 `work/<positionId>/`，生成 SKILL.md Territory 段，并把新角色的 `memoryScope` 写成 `./work/<positionId>/`；默认 `toolAllow` 只有 Read/Grep/Glob，不含 Write/Edit/Bash。employee.json 摘要密封机制不变。Refs #360。
