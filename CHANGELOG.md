...
- #284：统一工作台标题、按钮、选择项和卡片标签的居中对齐；修正 mint 与默认主题在明暗模式下的按钮、悬停态、禁用态和输入提示色，避免浅色背景上的浅色文字，并让设置表单与当前主题保持一致。文档标题、描述、链接及收藏标签采用可读配色，正文和代码保留阅读排版。群聊头像限制在既有容器内，避免遮挡消息和展开控件；展开消息中的链接保持可读。
