...
- #468：Goals / Reports overlay 处理回执（viewed / suggestion_applied / action_succeeded / action_failed / owner_resolved）。按钮点击不等于 resolved；成败混合分条展示且整项不自动解决。处理动作结果由 App 级运行时订阅既有 `turn.completed` / `turn.failed` / `turn.indeterminate` 与 `turn.approval.granted` / `turn.approval.denied`，按 workspaceKey+itemId+actionId 持久化，详情卸载后仍能写回执。回执只存在本地 workspace，不写 `goal.health`，不发给 Jev。无 overlay 时原界面不变。Refs #422 #428。
...
- #454：修复客户端审批模块状态一致性与重试缺陷。侧边栏角标仅统计未过期的待处理项；对话线程内审批卡片明确展示过期状态并禁用操作；批量裁决在失败或网络异常后清理请求锁以允许使用新理由重试；修复审批列表按本地日历日筛选跨时区偏差；刷新与裁决操作保证最新状态快照。

- #427：任务接口拒绝非法输入、未确认协作的状态变更和已结束任务的复活；公开执行轨迹只保留有界文件路径，避免工具原始参数泄露及中文路径导致回合中断；旧工作区附件随状态目录迁移后仍可读取。
...
- #456：审批中心体验增强与审计链集成。在抽屉中提供基于 LCS 的代码差异对比器（支持增删行高亮与行号）；支持多方审批进度条与「我的裁决」状态指示；审批队列支持单回合全选工具与逐项拒绝（非原子串行执行并保留失败项以便重试）；对话流卡片展示审批预览与风险等级；支持点击来源跳转并高亮聚焦对应回合（focusTurnId）；集成 GET /approvals/:id/audit 服务端验证审计事件展示。

- 新增独立「项目管理」模块，提供项目任务看板与排期：可拆分任务、分配岗位、设置优先级与起止日期，按任务阶段筛选，并查看关联 Agent 的真实执行状态。任务修改包含并发冲突检查，兼容既有目标数据。
- #427：新增持久化 Agent 任务看板、紧急插单、协作确认与外包预算归属；工作台显示分阶段启动进度和公开执行活动，RoleWeave 自有状态统一存储到 `.roleweave/`。
