...
- #456：审批中心体验增强与审计链集成。在抽屉中提供基于 LCS 的代码差异对比器（支持增删行高亮与行号）；支持多方审批进度条与「我的裁决」状态指示；审批队列支持单回合全选工具与批量拒绝；对话流卡片展示审批预览与风险等级；支持点击来源跳转并高亮聚焦对应回合（focusTurnId）；集成 GET /approvals/:id/audit 密码学哈希链校验与审计事件追踪。
