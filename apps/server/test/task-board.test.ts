...
test("task board preserves displaced work and enforces owner/direct/peer authority", async () => {
  ...
  const normal = await store.create(workspace, org, { actorPositionId: "lead", targetPositionId: "worker", title: "normal" });
  const peer = await store.create(workspace, org, { actorPositionId: "worker", targetPositionId: "lead", title: "ask" });
  const urgent = await store.create(workspace, org, { actorPositionId: "owner", targetPositionId: "worker", title: "urgent", urgent: true });
  ...
  const accepted = await store.decide(workspace, peer.taskId, org, { actorPositionId: "lead", decision: "accept" });
  assert.equal(accepted.status, "queued");
  await assert.rejects(store.create(workspace, org, { actorPositionId: "lead", targetPositionId: "worker", title: "fake urgent", urgent: true }), /only the owner/);
...
test("contractor tasks stay outside mainline and charge the delegating Agent budget", async () => {
  const workspace = await copyExampleWorkspace(); const store = new TaskBoardStore();
  const task = await store.create(workspace, org, { actorPositionId: "lead", targetPositionId: "worker", title: "edge research", contractor: true });
