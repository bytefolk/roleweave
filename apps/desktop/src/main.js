...
const { experimentsGet, experimentsUpdate, reportsAdvice, sendGateAdvice } = require("./experiments-ipc.cjs");
...
ipcMain.handle("owb:turn:send-gate-advice", async (event, request) => {
  if (!isTrustedWindowSender(event, mainWindow, trustedRendererUrl)) return { status: 403, body: { message: "Untrusted sender" } };
  return sendGateAdvice(request, apiRequest);
});
...
ipcMain.handle("owb:task:status", async (_event, request) => apiRequest(`/tasks/${encodeURIComponent(request.taskId)}/status`, { method: "PATCH", body: { status: request.status } }));
