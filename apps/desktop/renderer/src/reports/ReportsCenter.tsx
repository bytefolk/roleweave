...
function Escalations({ entries, positionNames, onOpenTimeline }: { entries: EscalationEntry[]; positionNames?: Record<string, string>; onOpenTimeline: (id: string) => void }) {
  ...
  return <ol>{entries.map((entry) => {
    ...
    return <li className="owb-report-card is-escalation" key={entry.turnId}><AlertOctagon aria-hidden="true" size={16} /><div><header><strong>{positionNames?.[entry.positionId] ?? t("rep.unknownPosition")}</strong><time title={formatTime(entry.at, localeTag)}>{formatRelativeTime(entry.at, localeTag, t)}</time></header><p className="owb-clamp-2" title={summary}>{summary}</p>{entry.category || entry.needsAttention !== undefined ? <p data-testid="rep-escalation-overlay">{entry.category ? t(`rep.jev.category.${entry.category}`) : null}{entry.needsAttention === false ? ` · ${t("rep.jev.needsAttentionNo")}` : entry.needsAttention === true ? ` · ${t("rep.jev.needsAttentionYes")}` : null}</p> : null}<div className="owb-report-chain">{entry.reportingChain.map((position, index) => <span key={position} style={{ borderLeftWidth: Math.min(index + 1, 4) }}>{positionNames?.[position] ?? t("rep.unknownPosition")}</span>)}</div><Button type="link" size="small" className="owb-report-trace" onClick={() => onOpenTimeline(entry.positionId)}>{t("rep.traceRun")}</Button></div></li>;
  })}</ol>;
}
