...
const copy = {
  ...
  preview: ["预览", "Preview"],
  description: ["在上报中心获得失败处理建议，也可在发送前请求人审建议。", "Get failure-handling suggestions in Reports and optional human-gate advice before sending."],
  ...
  ready: ["已开启 · 仅在你主动生成上报或发送前建议时运行本地推理。", "Enabled · Local inference runs only when you explicitly generate report or pre-send advice."],
  ...
  enabledNotice: ["已为当前项目开启。你可以在上报中心或发送前主动生成建议。", "Enabled for this project. Generate advice from Reports or before sending."],
  provider: ["服务提供方", "Provider"],
  sends: ["发送范围：执行状态、规范化错误码、是否与预算相关；发送前建议还会发送岗位 ID 与当前权限模式。", "Shared fields: run status, normalized error code, budget relation, and—only for pre-send advice—position ID and current permission mode."],
  excludes: ["不会读取输入框正文；任务摘要只在你逐次确认后发送。附件正文、员工姓名和项目路径不会发送。", "The composer body is never read. A task summary is sent only after per-request confirmation; attachment contents, employee names, and project paths remain excluded."],
