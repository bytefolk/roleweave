...
const copy = {
  ...
  preview: ["预览", "Preview"],
  description: ["在上报中心为失败执行提供处理建议；选中未就绪主机且存在多个就绪岗位时，也可自动建议切换。", "Get suggestions for failed runs in Reports. When the selected host is not ready and multiple ready positions exist, a host switch may also be suggested automatically."],
  ...
  unavailable: ["服务不可用：本机 Laya 服务尚未运行。启动服务后刷新状态。", "Service unavailable: The local Laya service is not available on this host. Start it, then refresh the status."],
  ready: ["已开启 · 上报中心需点击“生成建议”；未就绪主机且有 2 个及以上就绪岗位时会自动请求主机选择。", "Enabled · Reports still require Generate suggestions. A host Choice is requested automatically when the selected host is not ready and two or more ready positions exist."],
  enabledUnavailable: ["已开启 · 服务尚不可用", "Enabled · Service is not yet available"],
  restoreOff: ["恢复为关闭", "Restore to off"],
  off: ["已关闭 · 不会运行新的本地判断", "Off · No new local inference requests will run"],
  disabledNotice: ["已关闭，已有执行和上报记录保留。", "Turned off. Existing execution and report records are retained."],
  enabledNotice: ["已为当前项目开启。上报中心可生成建议；未就绪主机在有多个就绪岗位时会自动请求选择。", "Enabled for this project. Generate suggestions from Reports. A host Choice may run automatically when the selected host is not ready and multiple ready positions exist."],
  provider: ["服务提供方", "Provider"],
  sends: ["发送范围：执行状态、规范化错误码、是否与预算相关；主机选择另含岗位 ID、引擎、是否就绪。", "Shared fields: run status, normalized error code, and whether the issue is budget related. Host Choice also sends position ID, engine, and ready."],
  ...
  reportIntro: ["仅依据执行状态与错误类型提供建议。上述元数据只由本机 Laya 处理，不会发送到外部。", "Suggestions use run status and error type only. This metadata is processed only by Laya on this device."],
