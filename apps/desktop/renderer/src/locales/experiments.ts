...
const copy = {
  ...
  unavailable: ["服务不可用：本机 Laya 服务未就绪。请确认本机 Laya 已启动后刷新状态。", "Service unavailable: the local Laya service is not ready. Start Laya on this host, then refresh the status."],
  ...
  reportIntro: ["仅依据执行状态与错误类型提供建议。点击后将发送上述元数据至本机 Laya。", "Suggestions use run status and error type only. Generating them shares this metadata with local Laya."],
