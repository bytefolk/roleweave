...
const copy = {
  ...
  sends: ["发送范围：执行状态、规范化错误码、是否与预算相关；预算建议仅发送单任务剩余、单日剩余与岗位 ID。", "Shared fields: run status, normalized error code, and whether the issue is budget related; budget advice sends only per-task remaining, per-day remaining, and position ID."],
  ...
  budgetTitle: ["预算剩余建议", "Budget remaining advice"],
  budgetIntro: ["发送前可检查权威剩余额度；不会修改预算或自动发送。", "Check authoritative remaining quota before sending; this never changes budgets or sends automatically."],
  budgetCheck: ["检查预算建议", "Check budget advice"],
  budgetTaskRemaining: ["单任务剩余", "Per-task remaining"],
  budgetDayRemaining: ["单日剩余", "Per-day remaining"],
  budgetUnknown: ["暂无权威数据", "Authoritative data unavailable"],
  budgetUnknownAbstain: ["剩余额度不完整，Laya 已弃权", "Remaining quota is incomplete; Laya abstained"],
  budgetFailed: ["预算建议暂不可用", "Budget advice is temporarily unavailable"],
  budgetSuggestion: ["Laya 建议", "Laya suggestion"],
  budgetChoice_shrink: ["缩小未发送任务", "Shrink the unsent task"],
  budgetChoice_switch_employee: ["考虑其他员工", "Consider another employee"],
  budgetChoice_send_anyway: ["保持未发送任务", "Keep the unsent task"],
