...
const experiment = (enabled = true): ExperimentsResponse => ({ schemaVersion: "experiments.v1", workspacePath: "/projects/a", workspaceSession: "session-a", revision: 1, enabled, availability: enabled ? "ready" : "disabled", provider: { name: "Laya · local", endpointHost: "127.0.0.1", endpointUrl: "http://127.0.0.1:18081/v1/systemone", configured: true }, sending: ["status", "errorCode", "budgetRelated"], budgetAdviceSending: ["remainingPerTask", "remainingPerDay", "positionId"] });
...
const advice = (): ReportsAdviceResponse => ({ workspacePath: "/projects/a", workspaceSession: "session-a", revision: 1, status: "ready", items: [{ turnId: entry.turnId, positionId: entry.positionId, at: entry.at, suggestion: "inspect_run", source: "laya" }], cached: false, considered: 1, total: 1, generatedAt: entry.at });
