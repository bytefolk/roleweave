...
#447/#462 增量：项目实验设置 `GET/PATCH /experiments`、显式报告建议 `POST /reports/advice` 与发送前预算建议 `POST /turns/budget-remaining-advice` 的请求、工作区版本保护和外发数据约束见 [智能协作建议预览](laya-preview.md#控制面-api)。既有 `GET /reports` 保持本地事实读取，不触发外部服务。
...
事件词汇（v0 冻结 + D3 加法 + #25 加法 + #33 加法 + #480 加法）：`org.updated` / `turn.started` / `turn.model.delta` / `turn.usage` / `turn.completed` / `turn.failed` / `turn.indeterminate` / `turn.progress` / `turn.approval.requested` / `turn.approval.granted` / `turn.approval.denied` / `escalation.created` / `evidence.created` / `hire.progress`（#33 加法）。D3 的前五类引擎事件以已严格校验的 `engine.v1` 原始事件作为 `payload`；`turn.approval.*` 三类逐字镜像上游 #187 加法引入的 `engine.v1` `approval.requested` / `approval.granted` / `approval.denied` 事件（#25 Slice B 加法），经控制面同一严格校验后作为 `payload` 广播，不引入新词汇形态。进程级不确定结果使用控制面 `turn.indeterminate`，不会伪造 engine 终态，也不会自动重试。`hire.progress` 是控制面自产进展提示，`payload` 只含 `positionId` 与 `phase`（`validate` / `stage` / `apply`），不含百分比与终态；缺事件时前端停留上一相位，60 秒无任何事件按本地诊断码 `hire_timeout` 失败（不进入 §3 稳定码表，同 `turn_cancelled` 惯例）。`turn.progress`（#480）是控制面自产的管理者步骤进度，`payload` 为 `TaskProgressEvent`（`taskId`=`turnId`，`employeeId` 为 `positionId` 别名，工作台五步：thread-context / spawn / streaming / persist / terminal）。不含 chain-of-thought 与 engine NDJSON。快照另经 `GET /turns/progress` 与 `GET /turns/progress/:turnId` 读取，并落在 `.roleweave/conversations/<positionId>/progress/<turnId>.json`（独立于 TurnStore 的 `turns/*.json` 枚举）。
